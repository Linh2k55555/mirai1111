...
try{
	if (event.logMessageData.leftParticipantFbId == api.getCurrentUserID()) return;
	const { createReadStream, readdirSync, existsSync, mkdirSync } = require("fs-extra");
	const { join } = require("path");
	const { threadID } = event;
	const data = global.data.threadData.get(parseInt(threadID)) || (await Threads.getData(threadID)).data;
	const name = global.data.userName.get(event.logMessageData.leftParticipantFbId) || await Users.getNameUser(event.logMessageData.leftParticipantFbId);
	const type = (event.author == event.logMessageData.leftParticipantFbId) ? "tự rời" : "bị quản trị viên đá";
	const folderPath = process.cwd() + '/modules/commands/noprefix/';
	let msg, formPush;
	...
	if (files.length > 0) {
		...
		const attachment = createReadStream(join(folderPath, randomFile));
		formPush = { body: msg, attachment };
	} else {
		formPush = { body: msg };
	}
