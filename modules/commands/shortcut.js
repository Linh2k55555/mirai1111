        ...
        if (shortcut) {
            ...
            if (url) {
                ...
                switch (url) {
                    ...
                    case 'rd_girl':
                        return send(global.queuesabc.splice(0, 1));
                    case 'anime':
                        return send(global.queuesabc.splice(0, 1));
                    default:
...
module.exports.handleEvent = async function ({ api, event, Threads, Users }) {
    ...
    if (msg) {
        ...
        if (url) {
            ...
            else if (url === 'rd_girl') api.sendMessage({ body: msg, attachment: global.queuesabc.splice(0, 1) }, event.threadID, event.messageID);
            else if (url === 'anime') api.sendMessage({ body: msg, attachment: global.queuesabc.splice(0, 1) }, event.threadID, event.messageID);
            else {
