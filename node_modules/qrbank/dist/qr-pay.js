...
class QRBank {
    ...
    parse(content) {
        if (content.length < 4)
            return this.invalid();
        const crcValid = QRBank.verifyCRC(content);
        if (!crcValid)
            return this.invalid();
    ...
    build() {
        ...
        const version = QRBank.genFieldData(qr_pay_1.FieldID.VERSION, (_a = this.version) !== null && _a !== void 0 ? _a : '01');
        const initMethod = QRBank.genFieldData(qr_pay_1.FieldID.INIT_METHOD, (_b = this.initMethod) !== null && _b !== void 0 ? _b : '11');
        const guid = QRBank.genFieldData(qr_pay_1.ProviderFieldID.GUID, this.provider.guid);
        let providerDataContent = '';
        if (this.provider.guid === qr_pay_1.QRProviderGUID.VIETQR) {
            const bankBin = QRBank.genFieldData(qr_pay_1.VietQRConsumerFieldID.BANK_BIN, this.consumer.bankBin);
            const bankNumber = QRBank.genFieldData(qr_pay_1.VietQRConsumerFieldID.BANK_NUMBER, this.consumer.bankNumber);
        ...
        const provider = QRBank.genFieldData(qr_pay_1.ProviderFieldID.DATA, providerDataContent);
        const service = QRBank.genFieldData(qr_pay_1.ProviderFieldID.SERVICE, this.provider.service);
        const providerData = QRBank.genFieldData(this.provider.fieldId, guid + provider + service);
        const category = QRBank.genFieldData(qr_pay_1.FieldID.CATEGORY, this.category);
        const currency = QRBank.genFieldData(qr_pay_1.FieldID.CURRENCY, (_d = this.currency) !== null && _d !== void 0 ? _d : '704');
        const amountStr = QRBank.genFieldData(qr_pay_1.FieldID.AMOUNT, this.amount);
        const tipAndFeeType = QRBank.genFieldData(qr_pay_1.FieldID.TIP_AND_FEE_TYPE, this.tipAndFeeType);
        const tipAndFeeAmount = QRBank.genFieldData(qr_pay_1.FieldID.TIP_AND_FEE_AMOUNT, this.tipAndFeeAmount);
        const tipAndFeePercent = QRBank.genFieldData(qr_pay_1.FieldID.TIP_AND_FEE_PERCENT, this.tipAndFeePercent);
        const nation = QRBank.genFieldData(qr_pay_1.FieldID.NATION, (_e = this.nation) !== null && _e !== void 0 ? _e : 'VN');
        const merchantName = QRBank.genFieldData(qr_pay_1.FieldID.MERCHANT_NAME, this.merchant.name);
        const city = QRBank.genFieldData(qr_pay_1.FieldID.CITY, this.city);
        const zipCode = QRBank.genFieldData(qr_pay_1.FieldID.ZIP_CODE, this.zipCode);
        const buildNumber = QRBank.genFieldData(qr_pay_1.AdditionalDataID.BILL_NUMBER, this.additionalData.billNumber);
        const mobileNumber = QRBank.genFieldData(qr_pay_1.AdditionalDataID.MOBILE_NUMBER, this.additionalData.mobileNumber);
        const storeLabel = QRBank.genFieldData(qr_pay_1.AdditionalDataID.STORE_LABEL, this.additionalData.store);
        const loyaltyNumber = QRBank.genFieldData(qr_pay_1.AdditionalDataID.LOYALTY_NUMBER, this.additionalData.loyaltyNumber);
        const reference = QRBank.genFieldData(qr_pay_1.AdditionalDataID.REFERENCE_LABEL, this.additionalData.reference);
        const customerLabel = QRBank.genFieldData(qr_pay_1.AdditionalDataID.CUSTOMER_LABEL, this.additionalData.customerLabel);
        const terminal = QRBank.genFieldData(qr_pay_1.AdditionalDataID.TERMINAL_LABEL, this.additionalData.terminal);
        const purpose = QRBank.genFieldData(qr_pay_1.AdditionalDataID.PURPOSE_OF_TRANSACTION, this.additionalData.purpose);
        const dataRequest = QRBank.genFieldData(qr_pay_1.AdditionalDataID.ADDITIONAL_CONSUMER_DATA_REQUEST, this.additionalData.dataRequest);
        ...
        const additionalData = QRBank.genFieldData(qr_pay_1.FieldID.ADDITIONAL_DATA, additionalDataContent);
        const EVMCoContent = Object.keys((_f = this.EVMCo) !== null && _f !== void 0 ? _f : {}).sort().map(key => { var _a; return QRBank.genFieldData(key, (_a = this.EVMCo) === null || _a === void 0 ? void 0 : _a[key]); }).join('');
        const unreservedContent = Object.keys((_g = this.unreserved) !== null && _g !== void 0 ? _g : {}).sort().map(key => { var _a; return QRBank.genFieldData(key, (_a = this.unreserved) === null || _a === void 0 ? void 0 : _a[key]); }).join('');
        ...
        const crc = QRBank.genCRCCode(content);
        return content + crc;
    }
    static initQRBank(options) {
        const qr = new QRBank();
    ...
    async generateQRCode() {
        ...
        return await qrcode_1.default.toDataURL(content);
    }
    ...
    parseRootContent(content) {
        const { id, length, value, nextValue } = QRBank.sliceContent(content);
    ...
    parseProviderInfo(content) {
        const { id, value, nextValue } = QRBank.sliceContent(content);
        switch (id) {
    ...
    static verifyCRC(content) {
        ...
        const genCrcCode = QRBank.genCRCCode(checkContent);
